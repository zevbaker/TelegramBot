...
def locationInputHandler(chat_id, response):
    ...
    if resultCode == 200:   # Success city found
        ...
        message = cityName + ', ' + countryName + ': ' + str(temp_current) + degree_sign + 'C\n' + 'Max: ' + str(
            temp_max) + degree_sign + 'C - ' + 'Min: ' + str(temp_min) + degree_sign + 'C\n' + description_brief + ' - ' + description + emoji + emoji
...
def locationInputRequest(latitude, longitude):
    if not latitude or not longitude:
        logging.warning(
            'Error: - responseController.locationInputRequest: No lat or lng input')

    try:
        urlEncodePairs = {'lat': str(latitude), 'lon': str(
            longitude), 'APPID': WEATHER_API_KEY, 'units': WEATHER_UNIT, 'cnt': WEATHER_DAY_CNT}
        ...
        logging.warning(
            'Error: - responseController.locationInputRequest: ' + str(e))
...
def textInputRequest(text):
    if not text:
        logging.warning(
            'Error: - responseController.textInputRequest: No text input')

    try:
        urlEncodePairs = {'q': text, 'APPID': WEATHER_API_KEY,
                          'units': WEATHER_UNIT, 'cnt': WEATHER_DAY_CNT}
        encodedURL = urllib.urlencode(urlEncodePairs)
        ...
        logging.warning(
            'Error: - responseController.textInputRequest: ' + str(e))
...
def sendTextMessage(chat_id, text=None):
    ...
    try:
        message = urllib.urlencode({'chat_id': format(chat_id), 'text': text.encode(
            'utf-8', 'strict'), 'disable_web_page_preview': True})
        response = urllib.urlopen(
            BASE_URL + 'sendMessage', message.encode('utf-8'))

        logging.info('send response:')
        logging.info(response.read())
    except Exception as e:
        logging.warning(
            'Error - responseController.sendTextMessage: ' + str(e))
        return
